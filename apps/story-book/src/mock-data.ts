...
export const storyWorkspaces: WorkspaceInfo[] = [
  {
    ...
    displayName: "WorkspaceAgent App",
    path: "~/WorkspaceAgent/app",
    preset: "starter",
    workspaceType: "local",
  },
  {
    ...
    parts: [
      {
        type: "text",
        text:
          "Build a faithful story-book for the WorkspaceAgent app so we can iterate on the shell, session timeline, settings cards, and onboarding without touching the live runtime. Also make the mocked transcript feel closer to a real WorkspaceAgent session, including tool activity.",
      } as Part,
    ],
  },
  {
...
export const settingsCards = [
  {
    ...
    body: "Status for your local engine and WorkspaceAgent server with versioning, connection health, and repair actions.",
    points: [
      "OpenCode engine ready on localhost:4096",
      "WorkspaceAgent server proxied for remote workers",
...
export const onboardingChoices = [
  {
    ...
    detail: "Spin up a local WorkspaceAgent folder with starter automations and project memory.",
  },
  {
    title: "Connect remote worker",
    detail: "Attach to a hosted worker using WorkspaceAgent URL + token for shared remote execution.",
  },
];
