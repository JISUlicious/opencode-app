...
export function resolveTrustedSharedBundleFetchUrl(bundleUrl: unknown): URL {
  ...
  if (inputUrl.origin !== trustedBaseUrl.origin) {
    throw new ApiError(
      ...
      `Shared bundle URLs must use the configured WorkspaceAgent publisher (${trustedBaseUrl.origin}). Import only bundles from trusted sources.`,
    );
  }
  ...
  try {
    const response = await fetch(`${baseUrl}/v1/bundles`, {
      ...
      headers: {
        ...
        "X-WorkspaceAgent-Bundle-Type": bundleType,
        "X-WorkspaceAgent-Schema-Version": "v1",
        ...(input.name?.trim() ? { "X-WorkspaceAgent-Name": input.name.trim() } : {}),
      },
