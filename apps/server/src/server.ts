...
function withCors(response: Response, request: Request, config: ServerConfig) {
  ...
  headers.set(
    ...
    "Authorization, Content-Type, X-WorkspaceAgent-Host-Token, X-WorkspaceAgent-Client-Id, X-OpenCode-Directory, X-Opencode-Directory, x-opencode-directory",
  );
...
function createRoutes(
  ...
  addRoute(routes, "DELETE", "/workspaces/:id", "host", async (ctx) => {
    ...
    await recordAudit(workspace.path, {
      ...
      target: "workspace",
      summary: "Deleted workspace from WorkspaceAgent server",
      timestamp: Date.now(),
    });
  ...
  addRoute(routes, "POST", "/share/bundles/publish", "client", async (ctx) => {
    ...
    if (typeof body.baseUrl === "string" && body.baseUrl.trim()) {
      throw new ApiError(
        ...
        "Bundle publishing always uses the configured WorkspaceAgent publisher. Remove baseUrl from the request.",
      );
    }
