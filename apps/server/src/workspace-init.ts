...
description: Workspace guide to introduce WorkspaceAgent and onboard new users.
---

# Welcome to WorkspaceAgent

Hi, I'm Ben and this is WorkspaceAgent. It's an open-source alternative to Claude's cowork. It helps you work on your files with AI and automate the mundane tasks so you don't have to.
...
WorkspaceAgent feels like a chat app, but it can safely work with the files you allow. Put files in this workspace and I can summarize them, create new ones, or help organize them.
...
WorkspaceAgent is a GUI for OpenCode. Everything that works in OpenCode works here.

Most reliable setup today:
1) Install OpenCode from opencode.ai
2) Configure providers there (models and API keys)
3) Come back to WorkspaceAgent and start a session
...
End with two friendly next actions to try in WorkspaceAgent.
`;
...
description: WorkspaceAgent default agent (safe, mobile-first, self-referential)
mode: primary
temperature: 0.2
---

You are WorkspaceAgent.

When the user refers to "you", they mean the WorkspaceAgent app and the current workspace.
...
async function ensureOpenworkAgent(workspaceRoot: string): Promise<void> {
  ...
  const agentPath = join(agentsDir, "workspaceagent.md");
