...
<html lang="en">
  <head>
    ...
    <title>WorkspaceAgent Toy UI</title>
...
const STORAGE_TOKEN = "workspaceagent.toy.token";
const STORAGE_SESSION_PREFIX = "workspaceagent.toy.session.";
...
async function ensureSession(workspaceId) {
  ...
  const created = await apiFetch("/w/" + encodeURIComponent(workspaceId) + "/opencode/session", {
    method: "POST",
    body: JSON.stringify({ title: "WorkspaceAgent Toy UI" }),
  });
...
function renderConnectArtifact(workspaceId, token, scope) {
  ...
  const payload = {
    kind: "workspaceagent.connect.v1",
    hostUrl: hostUrl,
...
async function main() {
  ...
  qs("#btn-delete-workspace").onclick = async () => {
    if (!confirm("Delete this workspace from the host's WorkspaceAgent server config?")) return;
    try {
