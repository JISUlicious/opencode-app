  ...
  async function handleTelegramPairingGate(input: {
    ...
    if (!pairingCode) {
      await sendText(
        ...
        input.identityId,
        input.peerId,
        "This Telegram bot is private. Ask your WorkspaceAgent host for the pairing code, then send /pair <code>.",
        { kind: "system" },
      );
      return "handled";
    }
    ...
    if (!access.pairingCodeHash) {
      await sendText(
        ...
        input.identityId,
        input.peerId,
        "This Telegram bot is private but missing a pairing code. Ask your WorkspaceAgent host to reconnect it.",
        { kind: "system" },
      );
      return "handled";
    }
    ...
    if (!boundDirectoryCandidate || (!hasExplicitBinding && isDangerousRootDirectory(boundDirectoryCandidate))) {
      await sendText(
        ...
        input.identityId,
        input.peerId,
        "No workspace directory configured for this identity. Ask your WorkspaceAgent host to set it, or reply with /dir <path>.",
        { kind: "system" },
      );
      return "handled";
    }
  ...
  async function handleInbound(message: InboundMessage) {
    ...
    enqueue(key, async () => {
      ...
      try {
        ...
        const promptText = [
          "You are handling a Slack/Telegram message via WorkspaceAgent.",
