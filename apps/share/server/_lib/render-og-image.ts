...
function buildRootOgInput(): OgImageModel {
  return {
    title: "Share WorkspaceAgent skills beautifully",
    fileName: "agent-creator.md",
    fileType: "SKILL.md",
    description: "Clean metadata-first social cards for shared WorkspaceAgent skills and bundles.",
    category: "share",
...
function buildBundleOgInput({ rawJson }: { id: string; rawJson: string }): OgImageModel {
  ...
  const bundleName = maybeString(data.name).trim() || bundle.name || titleFromFileName(preview.filename) || "WorkspaceAgent bundle";
  ...
  const title =
    bundle.type === "workspace-profile"
      ...
      : bundle.type === "skills-set" && bundle.skills.length > 1
        ...
        : humanizeTitle(bundleName) || "WorkspaceAgent bundle";
...
export function computeOgImageLayout(model: OgImageModel): OgImageLayout {
  const displayTitle = truncateAtWordBoundary(humanizeTitle(model.title) || "WorkspaceAgent bundle", MAX_DISPLAY_CHARS);
...
function renderWorkspaceAgentLogo({ x, y, width, height }: { x: number; y: number; width: number; height: number }): string {
  return `
...
<svg width="${variantConfig.width}" height="${variantConfig.height}" viewBox="0 0 ${BASE_OG_IMAGE_WIDTH} ${BASE_OG_IMAGE_HEIGHT}" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  ${renderWorkspaceAgentLogo({ x: cardX + 66, y: cardY + 34, width: 40, height: 34 })}
