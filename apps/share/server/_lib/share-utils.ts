...
export function setCors(
  ...
  res.setHeader(
    ...
    options.headers ??
      "Content-Type,Accept,X-WorkspaceAgent-Bundle-Type,X-WorkspaceAgent-Schema-Version,X-WorkspaceAgent-Name",
  );
}
...
export function collectBundleItems(bundle: NormalizedBundle, limit = 8): PreviewItem[] {
  ...
  if (bundle.type === "workspace-profile") {
    ...
    if (maybeObject(bundle.workspace?.openwork)) {
      items.push({
        name: "workspaceagent.json",
        kind: "Config",
        meta: "WorkspaceAgent config",
        tone: "config",
      });
    }
...
export function buildBundlePreview(bundle: NormalizedBundle): {
  ...
  if (bundle.type === "skill") {
    return buildBundlePreviewSelection({
      ...
      text: buildTextPreview(bundle.content, `# ${bundle.name || "WorkspaceAgent skill"}`),
      tone: "skill",
  ...
  if (commands.length) {
    ...
    return buildBundlePreviewSelection({
      ...
      text: buildTextPreview(firstCommand.template || firstCommand.content, `# ${firstCommand.name || "WorkspaceAgent command"}`),
      tone: "command",
  ...
  if (maybeObject(bundle.workspace?.openwork)) {
    return buildBundlePreviewSelection({
      filename: "workspaceagent.json",
      text: buildJsonPreview(bundle.workspace?.openwork, '{\n  "openwork": {}\n}'),
      tone: "config",
      label: "WorkspaceAgent config",
    });
  }
  ...
  if (files.length) {
    ...
    return buildBundlePreviewSelection({
      ...
      text: buildTextPreview(firstFile.content, `# ${basename(firstFile.path) || "WorkspaceAgent portable file"}`),
      tone: preview.tone,
...
export function buildBundlePreviewSelections(bundle: NormalizedBundle): {
  ...
  return [
    {
      id: "preview-0",
      name: bundle.name || "WorkspaceAgent bundle",
...
export function buildBundleNarrative(bundle: NormalizedBundle): string {
  ...
  return parts.length
    ? `${parts.join(", ")} bundled into a worker package that imports through WorkspaceAgent with one step.`
    : "Worker configuration bundle prepared for WorkspaceAgent import.";
}
...
<head>
  ...
  <title>${escapeHtml(title)} - WorkspaceAgent Share</title>
  <style>
    @font-face {
