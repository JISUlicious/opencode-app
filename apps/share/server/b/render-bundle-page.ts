...
export function renderBundlePage({ id, rawJson, req }: { id: string; rawJson: string; req: RequestLike }): string {
  ...
  const title = bundle.name || `WorkspaceAgent ${typeLabel}`;
  const description = bundle.description || buildBundleNarrative(bundle);
  const items = collectBundleItems(bundle, 8);
  const compactItem = bundle.type === "skill" ? "skill.md" : items[0]?.name || "WorkspaceAgent bundle";
...
>
  <main class="shell">
    <nav class="nav">
      <a class="brand" href="/" aria-label="WorkspaceAgent Share home">
    ...
    <section class="hero-layout">
      <div class="hero-copy">
        ...
        <div class="hero-actions">
          <a class="button-primary" href="${escapeHtml(openInAppDeepLink)}">Open in WorkspaceAgent app</a>
          <a class="button-secondary" href="https://openworklabs.com/den" target="_blank" rel="noreferrer">Open in an WorkspaceAgent den</a>
        </div>
      </div>
      ...
      <div class="hero-artifact">
        <div class="app-window">
          <div class="app-window-header">
            ...
            <div class="app-window-title">WorkspaceAgent</div>
          </div>
    ...
    <section class="results-grid">
      <div class="result-card">
        <div class="step-list">
          <div class="step-row"><span class="step-bullet">01</span><span>Open the bundle in WorkspaceAgent</span></div>
          <div class="step-row"><span class="step-bullet">02</span><span>Choose the destination worker</span></div>
          <div class="step-row"><span class="step-bullet">03</span><span>Happy WorkspaceAgenting!</span></div>
        </div>
      </div>
    </section>
