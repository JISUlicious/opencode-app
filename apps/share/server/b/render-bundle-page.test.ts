...
test("renderBundlePage includes machine-readable metadata and escaped json script", () => {
  ...
  assert.match(html, /Open in WorkspaceAgent app/);
  assert.match(html, /Open in an WorkspaceAgent den/);
...
test("renderBundlePage shows workspace profile metadata", () => {
  ...
  assert.match(html, /Open the bundle in WorkspaceAgent/);
  assert.match(html, /Choose the destination worker/);
  assert.match(html, /Happy WorkspaceAgenting!/);
