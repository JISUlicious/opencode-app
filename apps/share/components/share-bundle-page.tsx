...
function humanizeLabel(value: string | null | undefined): string {
  ...
  if (!normalized) return "WorkspaceAgent Bundle";
...
function formatSurfaceEyebrow(selection: BundleSelection | undefined, parsedName: string, fallbackTitle: string | undefined): string {
  ...
  if (/^openwork\.json$/i.test(filename)) return "WorkspaceAgent Config";
...
export default function ShareBundlePage(props: BundlePageProps & { stars?: string }) {
  ...
  const openInWorkspaceAgent = (event: MouseEvent<HTMLAnchorElement>) => {
  ...
  const prepareOpenInWorkspaceAgent = (event: PointerEvent<HTMLAnchorElement> | MouseEvent<HTMLAnchorElement>) => {
    refreshOpenInAppHref(event.currentTarget);
  };

  const prepareOpenInWorkspaceAgentFromKeyboard = (event: KeyboardEvent<HTMLAnchorElement>) => {
  ...
  const actionButtons = (
    <>
      ...
      <a
        ...
        href={openInAppUrl}
        onPointerDown={prepareOpenInWorkspaceAgent}
        onMouseDown={prepareOpenInWorkspaceAgent}
        onKeyDown={prepareOpenInWorkspaceAgentFromKeyboard}
        onClick={openInWorkspaceAgent}
      >
        Open in WorkspaceAgent
      </a>
    </>
  );
    ...
    <main className="shell">
      ...
      {props.missing ? (
        <section className="status-card">
          <span className="eyebrow">WorkspaceAgent Share</span>
