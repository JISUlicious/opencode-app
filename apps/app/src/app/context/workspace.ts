...
export function createWorkspaceStore(options: {
  ...
  const DEFAULT_WORKSPACE_HOME_FOLDER_NAME = "WorkspaceAgent";
  ...
  const resolveOpenworkHost = async (input: {
    ...
    try {
      ...
      if (error instanceof OpenworkServerError && (error.status === 401 || error.status === 403)) {
        if (!trimmedToken) {
          throw new Error("Access token required for WorkspaceAgent server.");
        }
        throw new Error("WorkspaceAgent server rejected the access token.");
      }
      return { kind: "fallback" as const };
    }
    ...
    if (requestedWorkspaceId && !workspaceById) {
      throw new Error("WorkspaceAgent worker not found on that host.");
    }
    ...
    if (!workspace?.id) {
      throw new Error("WorkspaceAgent server did not return a worker.");
    }
    const opencodeUpstreamBaseUrl = workspace.opencode?.baseUrl?.trim() ?? workspace.baseUrl?.trim() ?? "";
    if (!opencodeUpstreamBaseUrl) {
      throw new Error("WorkspaceAgent server did not provide an OpenCode URL.");
    }
  ...
  const ensureBackendWorkspaceReady = async (
    ...
    if (!localServer) {
      throw new Error("Local WorkspaceAgent server is unavailable after opening the workspace.");
    }
    ...
    throw new Error("Local WorkspaceAgent server never registered the created workspace.");
  };
    ...
    if (remoteType === "openwork") {
      ...
      if (!hostUrl) {
        updateWorkspaceConnectionState(id, {
          status: "error",
          message: "WorkspaceAgent server URL is required.",
        });
        return false;
      }
      ...
      try {
        ...
        if (resolved.kind !== "openwork") {
          updateWorkspaceConnectionState(id, {
            status: "error",
            message: "WorkspaceAgent server unavailable. Check the URL and token.",
          });
          return false;
        }
        updateWorkspaceConnectionState(id, { status: "connected", message: null });
    ...
    try {
      if (isRemote) {
        ...
        if (remoteType === "openwork") {
          ...
          if (!hostUrl) {
            options.setError("WorkspaceAgent server URL is required.");
            updateWorkspaceConnectionState(id, {
              status: "error",
              message: "WorkspaceAgent server URL is required.",
            });
          ...
          try {
            ...
            if (resolved.kind !== "openwork") {
              options.setError("WorkspaceAgent server unavailable. Check the URL and token.");
              updateWorkspaceConnectionState(id, {
                status: "error",
                message: "WorkspaceAgent server unavailable. Check the URL and token.",
              });
  ...
  async function createSandboxFlow(
    ...
    setSandboxCreateProgress({
      ...
      steps: [
        ...
        { key: "health", label: "Wait for WorkspaceAgent", status: "pending", detail: null },
        { key: "connect", label: "Connect in WorkspaceAgent", status: "pending", detail: null },
      ],
    });
    ...
    try {
      ...
      // Ensure the workspace folder has baseline WorkspaceAgent/OpenCode files.
      const openworkServer = await resolveLocalOpenworkServer();
      ...
      try {
        stopListen = await listen(
          ...
          (event: TauriEvent<{ runId?: string; stage?: string; message?: string; payload?: any }>) => {
            ...
            if (stage === "workspaceagent.waiting") {
            ...
            if (stage === "workspaceagent.healthy") {
  ...
  async function updateRemoteWorkspaceFlow(
    ...
    const remoteType = normalizeRemoteType(workspace.remoteType);
    if (remoteType !== "openwork") {
      options.setError("Only WorkspaceAgent remote workers can be edited.");
      return false;
    }
  ...
  async function exportWorkspaceConfig(workspaceId?: string) {
    ...
    try {
      ...
      const outputPath = await saveFile({
        ...
        defaultPath,
        filters: [{ name: "WorkspaceAgent Worker", extensions: ["openwork-workspace", "zip"] }],
      });
  ...
  async function startHost(optionsOverride?: { workspacePath?: string; navigate?: boolean }) {
    ...
    if (!dir) {
      ...
      try {
        ...
        if (!result.found) {
          options.setError(
            ...
            ? "OpenCode CLI not found. Install the WorkspaceAgent-pinned OpenCode version for Windows or bundle opencode.exe with WorkspaceAgent, then restart. If it is installed, ensure `opencode.exe` is on PATH (try `opencode --version` in PowerShell)."
            : "OpenCode CLI not found. Install the WorkspaceAgent-pinned OpenCode version, then retry.",
        );
        return false;
      }
      ...
      if (!result.supportsServe) {
        ...
        options.setError(
          `OpenCode CLI is installed, but \`opencode serve\` is unavailable. Update to the WorkspaceAgent-pinned OpenCode version and retry.${suffix}`
        );
        return false;
      }
    } catch (e) {
