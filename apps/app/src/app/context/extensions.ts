...
export function createExtensionsStore(options: {
  ...
  const HUB_REPOS_STORAGE_KEY = "workspaceagent.skills.hubRepos.v1";
  ...
  const persistImportedCloudSkillHubs = async (
    ...
    if (!persisted) {
      throw new Error("WorkspaceAgent server unavailable. Connect to manage imported cloud skill hubs.");
    }
  ...
  const persistImportedCloudSkills = async (
    ...
    const persisted = await writeWorkspaceOpenworkConfigRecord(nextConfig);
    if (!persisted) {
      throw new Error("WorkspaceAgent server unavailable. Connect to manage imported cloud skills.");
    }
  ...
  const upsertWorkspaceSkill = async (
    ...
    if (isRemoteWorkspace) {
      throw new Error("WorkspaceAgent server unavailable. Connect to import skills.");
    }
  ...
  const deleteWorkspaceSkill = async (name: string) => {
    ...
    if (isRemoteWorkspace) {
      throw new Error("WorkspaceAgent server unavailable. Connect to remove skills.");
    }
  ...
  async function installHubSkill(name: string): Promise<{ ok: boolean; message: string }> {
    ...
    if (!canUseOpenworkServer) {
      if (isRemoteWorkspace) {
        return { ok: false, message: "WorkspaceAgent server unavailable. Connect to install skills." };
      }
      return { ok: false, message: "Hub install requires WorkspaceAgent server." };
    }
  ...
  async function refreshSkills(optionsOverride?: { force?: boolean }) {
    ...
    // Prefer WorkspaceAgent server when available
    if (canUseOpenworkServer) {
      if (root !== skillsRoot) {
        skillsLoaded = false;
      }
    ...
    if (!c) {
      setSkills([]);
      setSkillsStatus("WorkspaceAgent server unavailable. Connect to load skills.");
      return;
    }
  ...
  async function refreshPlugins(scopeOverride?: PluginScope) {
    ...
    if (!isLocalWorkspace && !canUseOpenworkServer) {
      setPluginStatus("WorkspaceAgent server unavailable. Connect to manage plugins.");
      setPluginList([]);
      setSidebarPluginStatus("Connect an WorkspaceAgent server to load plugins.");
      setSidebarPluginList([]);
      refreshPluginsInFlight = false;
      return;
    }
  ...
  async function installSkillCreator(): Promise<{ ok: boolean; message: string }> {
    ...
    // Use WorkspaceAgent server when available
    if (canUseOpenworkServer) {
      options.setBusy(true);
      options.setError(null);
    ...
    if (isRemoteWorkspace) {
      const message = "WorkspaceAgent server unavailable. Connect to install skills.";
      setSkillsStatus(message);
      return { ok: false, message };
    }
  ...
  async function readSkill(name: string): Promise<{ name: string; path: string; content: string } | null> {
    ...
    if (isRemoteWorkspace) {
      setSkillsStatus("WorkspaceAgent server unavailable. Connect to view skills.");
      return null;
    }
  ...
  async function saveSkill(input: { name: string; content: string; description?: string }) {
    ...
    if (isRemoteWorkspace) {
      setSkillsStatus("WorkspaceAgent server unavailable. Connect to edit skills.");
      return;
    }
