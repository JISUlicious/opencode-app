...
export function ServerProvider(props: ParentProps & { defaultUrl: string }) {
  ...
  const readStoredList = () => {
    try {
      const raw = window.localStorage.getItem("workspaceagent.server.list");
  ...
  const readStoredActive = () => {
    try {
      const stored = window.localStorage.getItem("workspaceagent.server.active");
  ...
  createEffect(() => {
    ...
    // In hosted web deployments served by WorkspaceAgent, OpenCode
  ...
  createEffect(() => {
    ...
    try {
      window.localStorage.setItem("workspaceagent.server.list", JSON.stringify(list()));
      window.localStorage.setItem("workspaceagent.server.active", active());
    } catch {
      // ignore
    }
  });
  ...
  const readOpenworkToken = () => {
    try {
      return (window.localStorage.getItem("workspaceagent.server.token") ?? "").trim();
    } catch {
      return "";
    }
  };
