...
export function createProvidersStore(options: CreateProvidersStoreOptions) {
  ...
  const persistImportedCloudProviders = async (
    ...
    if (!persisted) {
      throw new Error("WorkspaceAgent server unavailable. Connect to manage imported cloud providers.");
    }
  ...
  const cloudProviderComment = (provider: Pick<DenOrgLlmProvider, "id" | "name">) =>
    `// WorkspaceAgent Cloud import: ${provider.name.replace(/\s+/g, " ").trim()} (${provider.id}). Manage this entry from Cloud settings.`;

  const removeCloudProviderComment = (raw: string, providerId: string) =>
    raw.replace(
      new RegExp(
        `(^[ \t]*)// WorkspaceAgent Cloud import:.*\\n\\1(?="${escapeRegExp(providerId)}":)`,
        "m",
      ),
      "$1",
    );
  ...
  async function connectCloudProvider(cloudProviderId: string) {
    ...
    const orgId = settings.activeOrgId?.trim() ?? "";
    if (!token || !orgId) {
      throw new Error("Sign in to WorkspaceAgent Cloud and choose an organization first.");
    }
