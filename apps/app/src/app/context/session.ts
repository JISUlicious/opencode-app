...
export function createSessionStore(options: {
  ...
  const maybeAbortSyntheticControlLoop = (part: Part) => {
    ...
    const message =
      kind === "task-summary"
        ? "WorkspaceAgent stopped this run after detecting a likely synthetic task-summary loop. The engine kept asking itself to summarize task output and continue, which can repeat Goal/Instructions/Discoveries summaries without making progress."
        : "WorkspaceAgent stopped this run after detecting a likely auto-compaction continuation loop. The engine kept injecting synthetic continue prompts after compaction, which can burn tokens without advancing the task.";
