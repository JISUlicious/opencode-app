...
import { getWorkspaceAgentDeployment } from "../lib/openwork-deployment";
...
export default function SettingsShell(props: SettingsShellProps) {
  ...
  const webDeployment = createMemo(() => getWorkspaceAgentDeployment() === "web");
  ...
  const openFeedback = () => {
    const resolved = buildFeedbackUrl({
      ...
      deployment: getWorkspaceAgentDeployment(),
  ...
  const shareServiceDisabledReason = createMemo(() => {
    ...
    if (ws.workspaceType === "remote" && ws.remoteType !== "openwork") {
      return "Share service links are available for WorkspaceAgent workers.";
    }
    if (ws.workspaceType !== "remote") {
      const baseUrl = props.openworkServerHostInfo?.baseUrl?.trim() ?? "";
      const token =
        ...
        "";
      if (!baseUrl || !token) {
        return "Local WorkspaceAgent host is not ready yet.";
      }
    } else {
      const hostUrl = ws.openworkHostUrl?.trim() || ws.baseUrl?.trim() || "";
      const token = ws.openworkToken?.trim() || props.openworkServerSettings.token?.trim() || "";
      if (!hostUrl) return "Missing WorkspaceAgent host URL.";
      if (!token) return "Missing WorkspaceAgent token.";
    }
    return null;
  });
  ...
  const resolveShareExportContext = async (): Promise<{
    ...
    if (ws.workspaceType !== "remote") {
      ...
      const token =
        ...
        "";
      if (!baseUrl || !token) {
        throw new Error("Local WorkspaceAgent host is not ready yet.");
      }
      const client = createOpenworkServerClient({ baseUrl, token });
      ...
      if (!workspaceId) {
        throw new Error("Could not resolve this workspace on the local WorkspaceAgent host.");
      }
    ...
    if (ws.remoteType !== "openwork") {
      throw new Error("Share service links are available for WorkspaceAgent workers.");
    }

    const hostUrl = ws.openworkHostUrl?.trim() || ws.baseUrl?.trim() || "";
    const token = ws.openworkToken?.trim() || props.openworkServerSettings.token?.trim() || "";
    if (!hostUrl || !token) {
      throw new Error("WorkspaceAgent host URL and token are required.");
    }
    ...
    if (!workspaceId) {
      throw new Error("Could not resolve this workspace on the WorkspaceAgent host.");
    }
      ...
      <main class="min-w-0 flex-1 flex flex-col overflow-hidden rounded-[24px] border border-dls-border bg-dls-surface shadow-[var(--dls-shell-shadow)]">
        ...
        <ShareWorkspaceModal
          ...
          templateContentSummary={{
            ...
            commandNames: [],
            configFiles: ["opencode.json", "workspaceagent.json"],
          }}
