...
const STARTUP_SESSION_SNAPSHOT_KEY = "workspaceagent.startupSessionSnapshot.v1";
...
export default function App() {
  ...
  const SESSION_BY_WORKSPACE_KEY = "workspaceagent.workspace-last-session.v1";
  ...
  onMount(async () => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        if (!isTauriRuntime()) {
          const storedBaseUrl = window.localStorage.getItem("workspaceagent.baseUrl");
          if (storedBaseUrl) {
        ...
        const storedClientDir = window.localStorage.getItem(
          "workspaceagent.clientDirectory"
        );
        if (storedClientDir) {
        ...
        const storedEngineSource = window.localStorage.getItem(
          "workspaceagent.engineSource"
        );
        const storedEngineCustomBinPath = window.localStorage.getItem(
          "workspaceagent.engineCustomBinPath"
        );
        ...
        const storedOpencodeEnableExa = window.localStorage.getItem(
          "workspaceagent.opencodeEnableExa"
        );
        ...
        const storedUpdateAutoCheck = window.localStorage.getItem(
          "workspaceagent.updateAutoCheck"
        );
        ...
        const storedUpdateAutoDownload = window.localStorage.getItem(
          "workspaceagent.updateAutoDownload"
        );
        ...
        const storedUpdateCheckedAt = window.localStorage.getItem(
          "workspaceagent.updateLastCheckedAt"
        );
  ...
  createEffect(() => {
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("workspaceagent.baseUrl", baseUrl());
    } catch {
      // ignore
    }
  });
  ...
  createEffect(() => {
    ...
    try {
      window.localStorage.setItem("workspaceagent.projectDir", workspaceProjectDir());
    } catch {
      // ignore
    }
  });
  ...
  createEffect(() => {
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("workspaceagent.engineSource", engineSource());
    } catch {
      // ignore
    }
  });
  ...
  createEffect(() => {
    ...
    try {
      ...
      if (value) {
        window.localStorage.setItem("workspaceagent.engineCustomBinPath", value);
      } else {
        window.localStorage.removeItem("workspaceagent.engineCustomBinPath");
      }
    } catch {
      // ignore
    }
  ...
  createEffect(() => {
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("workspaceagent.engineRuntime", engineRuntime());
    } catch {
      // ignore
    }
  });
  ...
  const headerConnectedVersion = createMemo(() => {
    ...
    const normalizedVersion = openworkVersion.startsWith("v")
      ? openworkVersion
      : `v${openworkVersion}`;
    return `WorkspaceAgent ${normalizedVersion}`;
  });
    ...
    <OpenworkServerProvider store={openworkServerStore}>
      ...
      <ConfirmModal
        ...
        message={(() => {
          ...
          const configuredOrigin = warning?.configuredOrigin?.trim() || "the configured WorkspaceAgent share service";
          return `This link points to ${actualOrigin}, but WorkspaceAgent only auto-imports bundles from ${configuredOrigin}. Untrusted bundles can contain malicious instructions or settings. Only continue if you trust the sender and expect this import.`;
        })()}
