...
export function buildWorkspaceProfileBundle(
  ...
  return {
    ...
    description: "Full WorkspaceAgent workspace template with config, commands, skills, and portable .opencode files.",
    workspace: exported,
  };
}
...
export function buildSkillsSetBundle(
  ...
  return {
    ...
    description: "Complete skills set from an WorkspaceAgent workspace.",
...
export async function saveWorkspaceProfileBundleToTeam(input: {
  ...
  if (!token) {
    throw new Error("Sign in to WorkspaceAgent Cloud in Settings to share with your team.");
  }
