...
export function createBundlesStore(options: {
  ...
  const waitForBundleImportTarget = async (timeoutMs = 20_000, target?: BundleImportTarget) => {
    ...
    throw new Error("WorkspaceAgent worker is not ready yet.");
  };
  ...
  const createWorkerForBundle = async (request: BundleRequest, bundle: BundleV1) => {
    ...
    if (!hostUrl || !token) {
      throw new Error("Bundle link detected. Configure an WorkspaceAgent worker host and token, then open the link again.");
    }
    ...
    if (request.intent === "import_current") {
      ...
      if (!client || !connected || !hasTargetHint) {
        if (!bundleNoticeShown()) {
          ...
          options.setError("Bundle link detected. Connect to a writable WorkspaceAgent worker to import this bundle.");
        }
      ...
      if (!target.hostUrl.trim() || !target.token.trim()) {
        if (!bundleNoticeShown()) {
          setBundleNoticeShown(true);
          options.setError("Bundle link detected. Configure an WorkspaceAgent host and token to create a new worker.");
        }
      ...
      if (!resolveBundleImportTargetForWorkspace(workspace)) {
        disabledReason =
          workspace.workspaceType === "remote" && workspace.remoteType !== "openwork"
            ? "Only WorkspaceAgent-connected workers support direct bundle imports."
            : "This worker is missing the info WorkspaceAgent needs to import the bundle.";
      }
